...
backfitter = function(X, y, predictor, fitMethod, predictfcn, eps = .01, iter.max=10, verbose=TRUE,...){
  ...
  if(!is.numeric(predictor)){
    which_col = which(colnames(X)==predictor)
    Xc = X[, -(which_col)]
    Xs = as.vector(X[, which_col])	

	#Xc = X[, colnames(X)!=predictor]
	#Xs = as.vector(X[, colnames(X)==predictor])
	
#	#order by the predictor
#	xorder = order(X[, -which_col])
#	X = X[xorder, ]
#	y = y[xorder]
#	
  }
  {
	...
	#print(paste(nrow(X),nrow(Xc)))
	#print(paste(length(y), length(Xs)))
	...
	#print(paste("Backfitter Before:", nrow(Xc), length(y-g1_of_Xs)))
  ...
  times_to_repeat = as.numeric(table(Xs)) #remember Xs is sorted
	...
	#print("Done")
	#leaving us with...
	#print(paste("Backfitter After:", nrow(X), length(y-g1_of_Xs)))
